...
A difference in the reward structure of our DAG, when compared to a blockchain, is that not only the miner that solves a block gets a reward for that particular block; a number of the miners that confirm that block, later on, will receive a reward in the form of newly-created coins. This incentivizes the miners to reference all unreferenced blocks. Newly-created blocks that are not referenced by any other block yet are called leaves of the DAG.
