...
Byzantine behavior technically refers to random behavior but is commonly used to describe malicious actors diverging from the rules of the [protocol]({{ site.baseurl }}{% post_url /technology/expert/2022-01-03-a-protocol-to-transfer-value %}). An asynchronous environment means the protocol is run on a network, where there is no known bound to message delays. Messages can take an arbitrary time to reach their destination or can be dropped altogether.
...
In this article, we will look at an additional option to increase the throughput of a distributed ledger - changing the consensus mechanism. Many *Proof of X* schemes have been proposed over the last decade that aim to improve upon PoW in several ways. We want to give you a comprehensive overview with an appropriate amount of technical info in this article. We cannot do justice to every consensus mechanism, as this would fill an entire book in and off itself. But we do hope that after reading this article you will have a general orientation in the vast space of consensus research. 
