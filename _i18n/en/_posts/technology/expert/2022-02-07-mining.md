...
In a Sybil Attack an attacker creates a large number of malicious node in an effort to achieve some goal. In the blockchain context, this goal can be including malicious transactions in a block, censoring transactions from a given network participant or performing a double spend after a block reorganization.
