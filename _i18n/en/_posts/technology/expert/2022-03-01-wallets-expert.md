...
Wallets are for the most part applications that take care of key generation, storage and handling. In order to generate keys they first produce a random private key. Different sources for entropy generation can be used and in a best case several sources are combined.
A common back-up mechanism today is the mnemonic phrase, a list of words that can be used to restore your private key. Mnemonic phrases are generated by splitting the initial entropy into groups and mapping each group to a word from a list of words.

There are different ways to derive a public key from a private key and they differ in what kind of inputs they consume. Today most wallets are hierarchical deterministic wallets that can create several key pairs from an initial entropy source. This enhances the user experience as one only needs to back up a single key while being able to create several seperate accounts, even across different blockchains.

An advanced feature several wallets support are multi-signature transactions. They are classified as N-of-M schemes, where a minimum of N keys out of a set of M keys are needed to authorize spending of money. MultiSig wallets can be used to divide responisbility for storing money between several parties and hence increasing security. A single entity might also choose to setup a MultiSig account and store the keys in several locations.

We also looked under the hood of multi signature transactions. The conditions for spending from a multi signature account are defined in a redeem script. The hash of the redeem script is placed in the pubkey script of a transaction output where spending conditions are always defined. In order to spend money from such an account one needs the full (unhashed) redeem script, as well as the minimum amount of required digital signatures.
